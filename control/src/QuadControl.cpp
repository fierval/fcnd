...
VehicleCommand QuadControl::GenerateMotorCommands(float collThrustCmd, V3F momentCmd)
{
  ...
  cmd.desiredThrustsN[0] = (tx + ty - tz + c) / 4.f; // front left
  cmd.desiredThrustsN[1] = (-tx + ty + tz + c) / 4.f; // front right
  cmd.desiredThrustsN[2] = (tx - ty + tz + c) / 4.f; // rear left
  cmd.desiredThrustsN[3] = (-tx -ty - tz + c) / 4.f; // rear right
